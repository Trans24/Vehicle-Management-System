...
class Frontendbooking extends CI_Controller {
	...
	public function signup()
	{
		...
		$this->form_validation->set_rules('c_mobile', 'Mobile', 'required|min_length[10]|max_length[12]');
		...
		{ 
			...
			if($testxss){
				...
				if(count($exist)==0) {
					...
					if($response) {
						$this->session->set_flashdata('successmessage', 'Trans24 new Account created successfully..');
					} else {
				...
				$this->session->set_flashdata('warningmessage', 'Error! Your input are not allowed.Please try again with correct input');
				redirect('/');
			}
		}
	}
	public function login() 
	{
		...
		if ($query->num_rows() >= 1) {
			...
			if($result['c_isactive']==0) {
				$this->session->set_flashdata('warningmessage', 'You are not a active user .Please contact TRANS24 Manager');
				redirect('/');
			} else {
		...
		if($this->input->post('t_created_by')!='') {
			...
			if($this->form_validation->run() == FALSE) {
			  ...
			  redirect('/');
				...
				if($response) {
					$this->session->set_flashdata('successmessage', 'Your Booking is completed successfully.We will contact you shortly..');
				} else {
			...
			$this->session->set_flashdata('warningmessage', 'Please login before trying to book a vehicle');
			redirect('/');
		}
	}
