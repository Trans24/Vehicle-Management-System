...
class Drivers extends CI_Controller {
	...
	public function insertdriver()
	{
		...
		if($this->form_validation->run()==TRUE && $testxss){
			...
			if($response) {
				$this->session->set_flashdata('successmessage', 'Trans24 new driver added successfully..');
			    redirect('drivers');
			}
		...
		} else
		{
			...
			if(!$testxs) {
				$errormsg = 'Error! Your input are not allowed.Please try again with correct input';
			}
	...
	public function updatedriver()
	{
		...
		if($testxss){
			$response = $this->drivers_model->edit_driver($this->input->post());
				if($response) {
					$this->session->set_flashdata('successmessage', 'Trans24 Driver updated successfully..');
				    redirect('drivers');
				} else
				{
