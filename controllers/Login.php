		...
		$result = $this->login_model->check_login($this->input->post());
			if($result != FALSE)
			{
				...
				if($result['u_isactive']==0) {
					$this->session->set_flashdata('warningmessage', 'User not active.Please contact Trans24 Manager');
					redirect('login');
				} else if(empty($userroles)) {
					$this->session->set_flashdata('warningmessage', 'User role is not defined.Please contact Trans24 Manager');
					redirect('login');
				} else {
